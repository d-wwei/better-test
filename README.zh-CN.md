...
`better-test` 把它们固化——[Full Context + Lite Control](https://github.com/d-wwei/better-work) 框架的测试侧。持久化的测试手册、开发者反馈循环、经验提取机制，加上 4 层约束框架防止 agent 偷工减料。

## v2.0 新特性

v2.0 是一次重大重设计。核心新增：

- **4 层约束框架**（L0 目标校准、L1 Hook 硬约束、L2 子 Agent 独立验证、L3 人类审计面板）——确保 agent 不会声称假通过或跳过步骤
- **三层方法论架构**——核心流程嵌入 workflow（始终加载）、扩展流程按条件加载、设计文档供人阅读
- **测试执行框架**——结合通用纪律和项目专属知识，为每个项目生成专属的执行计划
- **经验提取**（`/better-test reflect`）——自动从测试历史中学习：验证 impact-map 映射、追踪稳定性趋势、识别 bug 热点、综合经验教训
- **差异测试**（`compare` 模式）——Rust 重写 vs C++ 原版对照，或版本间对比
- **Bug 生命周期管理**——结构化 bug 报告，状态追踪 OPEN → CONFIRMED → FIXED → VERIFIED → CLOSED
...
| L1 Hook | 5 个系统级 Hook（凭证扫描、空结果提问、执行日志...） | 机械错误：agent 可能忘记检查的 |
...
Cursor、Gemini CLI、Codex、OpenCode、OpenClaw 的适配安装命令见 `references/adapters.md`。测试知识文件是平台无关的。
...
- **约束框架 Hook 已设计但未实现。** L1-L3 需要配置 Claude Code hooks。
- **暂无 CI 集成。** GitHub Actions / GitLab CI 集成计划中。
...
系列完整介绍：[Full Context, Lite Control](https://github.com/d-wwei/better-work) 设计理念见系列入口 README。
