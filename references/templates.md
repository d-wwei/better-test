...
拆分后 init 只生成项目 protocol.md（安全纪律 + 项目纪律）。L0 + 思维纪律在 skill 的 `protocol-base.md`（不生成到项目）。
...
| 项目 | 必须满足 |
|------|---------|
| protocol-base.md 行数 | ~18 行（L0 ~12 + 思维纪律 ~4 + 标题/空行 ~2）— skill 维护，项目不改 |
| 项目 protocol.md 行数 | ≤ 15 行（安全纪律 ~3 + 项目纪律 ≤5 + 标题/空行 ~5）— 项目维护 |
| 合计（agent 每轮看到的） | ~33 行（两个文件之和） |
| L0 段 | 所有版本必须包含，不可省略 |
| 思维纪律段 | 所有版本必须包含，不可省略 |
...
### Added
- 安全纪律（<严格/标准/宽松> 按风险等级选择）
- 项目纪律段（初始为空）
- 注：L0 + 思维纪律在 skill 的 protocol-base.md，不记入项目 protocol changelog
...
**run 级** `run-<tester-id>-NNN-<ts>/status.md` 是该 tester 本 run 的测试状态，由 tester 自己维护。
...
## registry.md（tester 注册表）

每个 tester 的轻量注册文件，存放于 `testers/<tester-id>/registry.md`。是 tester 在 `testers/` 目录下的**唯一文件**。

**职责**：身份标识 + 资源声明 + run 索引。不存放测试结果或进度（那些在 run 目录内）。
...
| Resource | Value |
|----------|-------|
| daemon_port | <port number> |
| daemon_pid | <PID> |
| config_path | <path> |

## Runs

| # | Version | Run | Status | Time |
|---|---------|-----|--------|------|
| 1 | v<X.Y.Z> | run-<tester-id>-001-<ts> | completed | <MM-DD HH:MM±ZZ> |
| 2 | v<X.Y.Z> | run-<tester-id>-002-<ts> | in-progress | <MM-DD HH:MM±ZZ> |
```

**写入时机**：
- 注册时创建（strategy Step 0）
- 资源变更时更新 Resources 段（如重启 daemon 换端口）
- run 完成时更新 Runs 表状态
...
| 项目 | 必须满足 |
|------|---------|
| Resources 段 | 列出所有占用的共享资源（端口、PID、配置路径） |
| Runs 表 | 每个 run 有对应的 run 目录路径 |
| 总长度 | ≤ 30 行（只做索引不做内容） |
| 不含凭证 | 不写密码/token/API key |

---

## bio.md（run 级身份快照）

每次 run 的 tester 身份快照，存放于 `run-<tester-id>-NNN-<ts>/bio.md`。**创建后不可变**——所有动态信息（进度、发现）走 progress.md。
...
|-------|---------|----------|
| **Register** | 首次 `strategy` / `checkpoint`（session 无活跃 tester） | 探测平台 + session 信息，创建 registry.md + run 目录 + bio.md |
| **Active** | 每次执行命令 | 更新 registry.md `last_active`，写 status/progress 到 run 目录 |
| **Resume** | `/better-test resume` | 扫描 registry.md 找到 run 列表，读 run 目录内 progress.md 恢复 |
...
Strategy workflow Step 4-5 的输出持久化。存放于 `run-<tester-id>-NNN-<ts>/strategy-plan.md`。
...
strategy-plan.md 直接在 run 目录内创建，无需归档复制。
...
│       │
│       └── merge-<coordinator-id>-<ts>/    ← coordinator 工作目录（仅多 tester 时）
...
| `bugs/BUG-NNN-<slug>.md` | 开发者（修 bug） | 每个 bug 的完整技术细节，结构化 | 发现 bug 时写入 |
...
| 项目 | process-log | bugs/BUG-NNN-slug | summary |
|------|------------|--------------|---------|
...
```yaml
bug:
  id: BUG-NNN                # run 内编号；merge 后由 coordinator 分配 BUG-<version>-NNN
  title: <简短标题>
...
```
1. 测试中发现 bug
   → agent 写 bug report 到 run-<tester-id>-NNN-<ts>/bugs/BUG-NNN-slug.md（run 内编号）
   → status: OPEN
   → results.json 中相关 items 的 bug_ids 填入 BUG-NNN
   → 单 tester: 直接更新 bugs-index.md；多 tester: 由 coordinator merge 时合并
...
| 场景 | 用什么机制 | 写哪里 |
|------|-----------|--------|
| 开发者对某个**测试项**的判定 | `/better-test feedback <test_id> <verdict>` | `run-*/feedback/<id>_<verdict>.md`（merge 时重建 `feedback-rules.json`） |
| 开发者修了某个 **bug** | 更新 bug report 的 yaml status | `run-*/bugs/BUG-NNN-slug.md` → 触发 bug-retest |
...
### bugs-index.md（跨版本 bug 索引，derived view）

bug 可能跨版本（v1.4.28 发现，v1.4.29 修复，v1.4.30 验证），需要全局索引。

**由 coordinator 通过 `/better-test merge` 生成**，从 `merge-*/bugs/` 和各 `run-*/bugs/` 汇总。单 tester 场景由 tester 在 run 完成后自行生成。

```markdown
# Bug Index

> derived view — 由 /better-test merge 生成，勿手编。

| ID | 标题 | 状态 | 类型 | 严重 | 发现版本 | 修复版本 | 关联测试 | 来源 run | 回归 canary |
|----|------|------|------|------|---------|---------|---------|---------|------------|
| BUG-v1.4.28-001 | option chain 窝轮返回空 | VERIFIED | integration | P1 | v1.4.28 | v1.4.29 | C-05, C-07 | run-claude-a3f2-002 | ✓ |
| BUG-v1.4.28-002 | auth timeout on slow net | OPEN | environment | P2 | v1.4.28 | — | A-03 | run-codex-c9d4-001 | — |
```

**Bug 编号规则**：
- **run 内**：`BUG-NNN-<slug>.md`（NNN 在 run 内递增，从 001 开始）
- **项目级**（coordinator 合并后）：`BUG-<version>-NNN-<slug>.md`（NNN 全局递增）
...
| 项目 | 必须满足 |
|------|---------|
| Bug ID (run 内) | `BUG-NNN-<slug>` 格式，NNN 在 run 内递增 |
| Bug ID (项目级) | `BUG-<version>-NNN-<slug>` 格式，NNN 全局递增 |
| Bug status | 只通过生命周期流程更新，不跳阶段（不能 OPEN → CLOSED） |
| related_tests | 必须是 test-groups.md 中存在的 ID |
| run 目录完整性 | 每个 run 目录必须包含 bio.md + results.json；如有 L2/L3 则一并写入 |
...
## Coordinator 输出文件（merge-<coordinator-id>-<ts>/）

由 `/better-test merge` 生成。详见 `references/merge-workflow.md`。

### merge bio.md（coordinator 身份）

```markdown
---
coordinator_id: coordinator-<platform>-<4hex>
platform: <claude-code | codex | cursor | gemini | unknown>
model: <model identifier>
created: <YYYY-MM-DDTHH:MM:SS±HH:MM>
---

# Coordinator: coordinator-<platform>-<4hex>
...
| # | Run | Tester | Version | Result |
|---|-----|--------|---------|--------|
| 1 | run-claude-a3f2-002-... | claude-a3f2 | v1.4.28 | 12/14 pass |
| 2 | run-codex-c9d4-001-... | codex-c9d4 | v1.4.28 | 8/8 pass |
```

### conflict-log.md（冲突记录）

```markdown
# Conflict Log — <MM-DD HH:MM:SS±ZZ>
# Coordinator: <coordinator-id>

## Coverage Gaps

| Planned Stage | Planned By | Tested By | Status |
|---------------|-----------|-----------|--------|
| Stage 4: D 组 | claude-a3f2 | (nobody) | GAP — 未测试 |

## Result Conflicts

| test_id | Tester A | Result A | Tester B | Result B | Analysis | Resolution |
|---------|----------|----------|----------|----------|----------|------------|
| B-05 | claude-a3f2 | pass | codex-c9d4 | fail | 时序差异：A 先测 daemon 正常，B 后测 daemon 已重启 | 标 fail（保守） |

## Bug Conflicts

| test_id | Tester A | Tester B | Analysis |
|---------|----------|----------|----------|
| (none) | — | — | 无 bug 冲突 |
```
...
```markdown
# 合并测试速览 — v<X.Y.Z>
# <MM-DD HH:MM:SS±ZZ> | <N> 个 run 合并 | Coordinator: <coordinator-id>
...
## 一句话结论
<如："综合两个 tester 结果，发现 2 个 bug，1 个结果冲突（B-05，标 fail）。覆盖率 18/20 = 90%。">
...
- 聚合结果: merged-results.json
- 各 tester 原始数据: [run 目录列表]
```
