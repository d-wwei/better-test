...
```
如存在以下材料，reflect 应优先纳入：
  - test/reference/retrospective-*.md
  - test/history/**/merge-*/merge-summary.md
  - test/history/**/SESSION-RETROSPECTIVE.md
  - cross-verify 记录 / disagreement resolution 文档

优先级：
  raw artifact（JSON / log / execution-log）
    > 结构化 run 输出（results / bug reports / l2-findings）
    > retrospective / merge summary
    > summary prose

复盘文档的用途：
  - 提取可复用的方法论
  - 识别需要升级到 workflow / queue 的模式
  - 不能单独推翻 raw evidence
```
...
```
输入：
  - 所有 results.json 的时间戳（started_at / finished_at，带时区偏移）
...
## 分析 7：Skill 升级队列账实对账（2026-06-11 新增）

> 背景：队列曾出现 6 条"内容已写进 skill 但状态仍 pending"的账实不符——promote 与状态更新
> 一旦分离，队列就不再是可信的事实来源。全量 reflect 时核对一次。

```
读 skill 的 references/pending-skill-upgrades.md：

1. 统计 pending 条数 → ≥5 输出醒目警告（违反 update-workflow Step 5.5 水位红线，建议立即清队）
2. 对每条 pending：取其"内容"的 2-3 个特征短语，grep skill 的 references/ 全部文件
   → 命中（内容已存在）→ 建议状态改 already-present + 标注所在文件
   → 未命中 → 保持 pending
3. 对每条 promoted：抽查其声称的落点文件确实包含该内容 → 不含则标 ⚠ 状态虚标
4. 入队超过 30 天仍 pending 的条目 → 列入"陈化清单"提示用户批/拒/撤

输出：对账差异列表（需用户确认后更新队列状态）
```
...
```markdown
# Reflect 报告 — <MM-DD HH:MM:SS±ZZ>

## 数据范围
分析了 <N> 次运行（<version_range>），<M> 个 bug，<K> 条 feedback
涉及 tester: <tester-id 列表>

## 跨 Tester 发现

| 发现 | 详情 |
|------|------|
| 平台差异 | <e.g., "D-03 在 claude-a3f2 上 pass 但在 codex-c9d4 上 fail"> |
| 模型差异 | <e.g., "opus-4-6 平均每组多跑 30s 但 bug 发现率更高"> |
| 设备差异 | <e.g., "CI runner 上 flaky 率 20%，本地 0%"> |
| 效率对比 | <e.g., "codex 单次 full regression 耗时 45min，claude 55min"> |
...
**共享知识文件**（test-groups.md、impact-map.md、env-config.md）：任何时候可直接写。

**Derived view 文件**（known-issues.md、status.md）：
- **测试期间**（有活跃 session 文件）：reflect 发现写到 run 目录内 progress.md 关键发现段，由 `/better-test merge` 或单 tester 完成后合并到项目级文件
- **非测试期间**（无活跃 session、如独立跑 `/better-test reflect`）：可直接写项目级文件
...
- reflect 建议新增 lesson → 多次出现只说明“重复观察”，不能自动标 confirmed；
  只有至少两份不同 `independence_key` 且落在两个不同 run-local artifact 文件的直接证据
  才标 `confirmed`；源码/proto 或跨版本根因验证还要写结构化 `proven_basis`，functional
  结论另有独立 runtime artifact，才标 `proven`
