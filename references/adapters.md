...
把项目的 better-test 知识根目录注入到不同 agent 平台。

## 设计原则

- 每个项目只允许一个测试知识根目录；默认是 `.better-work/test/`
...
新项目继续使用 `.better-work/test/`。已有项目若把知识直接放在 `test/`，在项目根提交
`.better-test-root`，内容只写一行仓库相对路径：

```text
test
```

这是 flat 项目的推荐配置：可进 Git、Codex 和 Claude 共用，不依赖每台机器手工导出环境变量。
未配置时，hooks 仍会在 `test/protocol.md` 存在，且同时存在 `test-groups.md`、`status.md` 或
`history/` 之一时自动识别 flat root。仓库外或临时目录才使用绝对路径环境变量：

```bash
export BETTER_TEST_DIR=/absolute/path/to/project/test-assets
```

解析顺序是：`BETTER_TEST_DIR` → 最近的 `.better-test-root` → 最近的 `.better-work/test/` → 带上述标记的 flat `test/` →
`~/.better-work/<project>/test/`。一个项目不要同时维护 `.better-work/test/` 和 `test/` 两套内容。
下文示例默认写 `.better-work/test/`；flat 项目应把项目级引用替换为 `test/...`。Skill 自身路径不变。
...
只保留一个 canonical Git checkout，并从该目录运行安装器。Claude、Codex legacy 路径和 Codex
canonical 路径都必须是指向同一 checkout 的 symlink；不要把仓库复制进各平台的 skills 目录。
`./install.sh status` 会显示 canonical 路径、完整 Git commit 和 clean/dirty 状态。若目标位置已有真实
目录，安装器会拒绝覆盖并标记为 `unmanaged`；先人工备份或迁移该目录，再重新执行安装。
...
`install` 和 `status` 都会打印解析后的 `test root:`。flat 项目看到该值以 `/test` 结尾，才算路径适配成功。

行为约束：

- 默认安装到项目 `.codex/hooks.json`
- 自动识别当前 Codex 的 feature 名称（0.145+ 为 `hooks`，旧版为 `codex_hooks`），并接受 runtime 已默认启用的 stable feature
- 若未启用，会报错退出
