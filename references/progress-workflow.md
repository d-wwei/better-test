...
Agent 检测到以下情况时，**必须**执行或建议 checkpoint：

1. **[必做]** 每完成一个 Phase/测试组时 → 立即更新 progress.md（最低一两行："当前第 X 阶段，已完成 Y，下一步 Z"）。strategy-plan.md 和 process-log.md 是防漂移锚点，不是文档负担
2. 已经连续跑测试超过 30 分钟且有实质进展（多组完成）
3. 对话长度接近 context 压缩阈值
