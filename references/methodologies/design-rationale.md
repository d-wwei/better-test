...
API 契约里每个可选字段应至少测 4 种状态：key 缺失 / key 存在但值为空 / key 存在且值为 false(或 0、空串这类有效假值) / key 存在且合法非空。只测第 4 种 = 只测 happy path。boundary testing 完全缺席，以及把 `false` 当 absent，是 agent 最常见的盲区。

## 八、测试体系六部件（完整性框架）

> 来源：futu-opend-rs 2026-06 体系审计。用途：回答"我的测试体系还缺什么"——
> 逐部件机械核对，而非凭感觉。一个部件缺失的典型症状附后。

完善的测试体系 = 六个部件的乘积，任何一项为零整体为零：

| 部件 | 回答的问题 | 缺失症状 |
|------|-----------|---------|
| 1. 风险模型 | 测试投资往哪儿砸？ | 覆盖跟着开发者 changelog 走（确认偏误）；平均用力 |
| 2. 分层结构 | 每类问题在哪一层拦截？ | 贵的层（真实交易/真机）验证便宜层（log/binary）就能验的事 |
| 3. 测试设计技术 | 用例怎么系统地生成？ | 用例靠拍脑袋；符号/参数硬编码单一代表；整个等价类零覆盖 |
| 4. Oracle 体系 | 怎么知道结果是对的？ | 断言停留在"返回非空"；修复方向无对照被定向验证通过 |
| 5. 度量与出口准则 | 什么叫测完了？ | 覆盖率无分母；"矩阵存在但不知是否执行"；发版无 DoD |
| 6. 反馈回路 | 体系自己怎么变好？ | 同类 bug 反复逃逸；经验沉淀无人读；升级队列积压无人审 |

部件 6 的双环结构：单环 = bug → 回归用例（feedback workflow）；
双环 = 逃逸 → 追问"体系为什么没拦住" → 修流程（escape analysis，见 feedback-workflow）。
单环人人会做，双环才能阻止同类逃逸。

质量维度清单（功能/鲁棒/安全/性能/一致性/可恢复/可观测/数据/兼容/配置/并发/DX/合规/i18n，
见 init-workflow 维度评估）只是部件 1 的输入——维度回答"测什么"，六部件回答"体系怎么转"。
