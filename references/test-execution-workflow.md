...
L1 Hook（执行日志记录）自动把每条 Bash 命令 + 输出追加到 `.better-work/test/testers/<tester-id>/execution-log.md`。Agent 不需要手动记录执行过程——Hook 替它做。
...
L1 Hook 自动记录每条 Bash 命令和输出到 `test/execution-log.md`。Agent 不需要手动做任何事——Hook 替它做。这是不可篡改的执行记录，L2 审计的数据源。
...
2. 生成 2 分钟速览 → history/<version>/run-<tester-id>-NNN-<ts>/summary.md
3. 汇总 bug reports → history/<version>/bugs/（每个 bug 独立文件，ID 含 tester-id）
4. 触发 L2 独立验证（读 references/l2-audit-prompts.md，spawn 子 Agent）
   → 子 Agent 读 execution-log + results + manifest + known-issues
   → 输出 l2-findings.md
5. 基于 l2-findings 生成 audit-report.md
6. 归档到 history：
   test/execution-log.md             → 复制到 history/<ver>/run-<tester-id>-NNN-<ts>/execution-log.md
   testers/<tester-id>/l2-findings.md → 复制到 history/<ver>/run-<tester-id>-NNN-<ts>/l2-findings.md
   audit-report.md                    → 复制到 history/<ver>/run-<tester-id>-NNN-<ts>/audit-report.md
   process-log.md（如存在）           → 复制到 history/<ver>/run-<tester-id>-NNN-<ts>/process-log.md
   testers/<tester-id>/ 下原文件保留（供快速读取最近状态）
8. 向用户呈现（按顺序）：
