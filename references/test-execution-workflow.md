...
## [MM-DD HH:MM:SS±ZZ] <卡在哪 / 改变了什么判断>
<原因>
```

**关键原则**：
- **只追加不删改** — 即使后面推翻了之前的结论，也不回去改前面的记录。新结论追加在后面，标注"推翻了 [MM-DD HH:MM:SS±ZZ] 的判断"
...
2. 归档 process-log.md → history/<version>/run-<tester-id>-NNN-<ts>/process-log.md
3. 生成 2 分钟速览 → history/<version>/run-<tester-id>-NNN-<ts>/summary.md
4. 汇总 bug reports → history/<version>/bugs/（每个 bug 独立文件，ID 含 tester-id）
...
| `bugs/BUG-<tester-id>-NNN.md` | 开发者（修 bug 用） | 每个 bug 的完整技术细节，结构化整理，不重复 | 7 节模板（procedures/bug-report.md），证据确凿，可复现 |
