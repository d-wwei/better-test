...
- **`/better-test merge` command**: New coordinator workflow (`references/merge-workflow.md`) with 9 steps — scan runs, register coordinator, read results, coverage audit, conflict detection, bug reconciliation, feedback merge, generate output, user confirmation
- **`testers/<id>/registry.md`**: Lightweight tester registry replacing the old `bio.md` in testers/. Contains identity, resource declarations (daemon ports, PIDs), and run index
- **`run-*/bio.md`**: Immutable run-level identity snapshot (replaces tester-level bio.md)
- **Write permission matrix**: Tester can only write to `testers/<own>/registry.md` + `run-<own>-*/` during testing; coordinator is the sole writer of project-level aggregation files
- **Registration gate**: Must verify registry.md + run/bio.md exist before testing begins (Red Line #13)
- **Resource awareness**: Strategy Step 0 scans other testers' registry.md Resources to show port/daemon conflicts
- **Bug numbering**: Run-local `BUG-NNN` during testing; coordinator assigns project-level `BUG-<version>-NNN` during merge
- **Merge directory**: `merge-<coordinator-id>-<ts>/` stores coordinator's bio, conflict-log, merged-summary, merged-results, reconciled bugs

### Changed

- **Output Structure (SKILL.md)**: Major rewrite — bugs/ and feedback/ moved into run directories; status.md/known-issues.md/bugs-index.md/feedback-rules.json marked as derived views
- **strategy-workflow.md**: Tester registration creates registry.md + run directory + bio.md; added registration gate, resource awareness, write permission matrix; Step 5.5 writes to run directory
- **templates.md**: bio.md split into registry.md (mutable) + run-level bio.md (immutable); history directory restructured; archive flow removed (no longer needed); bug numbering updated
- **progress-workflow.md**: Checkpoint writes to run directory; resume scans registry.md for run list; added resource conflict check on resume
- **feedback-workflow.md**: Feedback writes to run-internal feedback/ directory; feedback-rules.json no longer updated directly (rebuilt by coordinator)
- **Red Lines**: Added #12 (tester writes aggregation files = violation), #13 (registration gate), #14 (cross-tester write = violation); removed old #11 (bio working notes limit, bio is now immutable)
- **Acceptance Criteria #6**: Updated to specify tester isolation + coordinator merge verification

### Removed

- **Archive flow**: No longer needed — testers write directly to run directories instead of testers/ → history/ copy
- **Auto-refresh of status.md**: Removed "strategy/update/feedback auto-refresh" — project-level status.md only generated by merge or single-tester completion

### Design rationale

Driven by real failure: agents writing to shared files concurrently during parallel testing. Root cause analysis showed 4 shared files (status.md, bugs-index.md, known-issues.md, feedback-rules.json) are all derivable from isolated primary data. Solution: make them derived views, only written by a coordinator role after all testers complete. Single tester doesn't need merge — run output is the project output.
